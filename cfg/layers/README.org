...
- ~services~ :: *optional* used to inject "service" type component config
  ...
  explicit.
- ~options~ :: *optional* way to provide options related to *end user*.  The
  ~options~ *are not* meant to provide variations on a detector but for to
  specify settings required for a *mid-layer API* to properly connect to
  things not covered by the API.  The example given is if frames
  should be expressed in a sparse manner.  The allowed ~options~ may be
  specific to a detector (but general to all variants of that
  detector).  The user may omit ~options~ and the API must assume
  reasonable defaults.
