...
In the tips and tricks below, we will use ~WCTEST_TMPDIR~ to implement /idempotent/ tests.
#+end_note
...
Nominally, ~run_idempotently~ will always run the command as the ~bats~ command will run the tests in a fresh temporary context.  Thus to allow idempotent running the user must set ~WCTEST_TMPDIR~.  As that directory may start empty, the first test run will always run the ~<command line>~ while subsequent runs may exhibit idempotency. 
