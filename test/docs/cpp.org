...
#+end_example
The ~<prefix>~ may match any of the groups named in section [[Test groups]].  The developer is free to write tests of these types following the guidelines give by the WCT testing.
...
If you have not yet done so, read the section [[Framework]] for an overview and section [[Writing tests]] for general introduction to writing tests.
The remaining sections describe how to write WCT tests in C++.
