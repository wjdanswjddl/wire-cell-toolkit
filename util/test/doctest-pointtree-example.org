...
* From k-d tree to n-ary tree :noexport:

In the above section we showed how to navigate from a point in a k-d tree result to the local point cloud that provided the point by calculating a *major index* and iterating that same number of elements from the beginning of the ~disjoint_range~.  The same approach can be used to find the n-ary tree node that provided the local point cloud that provided a point.  In this patter we replace the ~disjoint_range~ iterator with a ~NaryTree::depth_iter~.  To show this we will repeat most of the above test case

~doctest-pointtree-example.cxx~
...
#+begin_src c++ :tangle no
    ...
    TEST_CASE("point tree example scoped k-d tree to n-ary nodes")
    {
        ...
        // Get the depth iterator/range.  See text.
        auto dr = root->depth(scope.depth);
    
        // In one case we prefill an ordered vector of nodes
        std::vector<Points::node_t*> scope_nodes;
        for (auto& pval : dr) {
            scope_nodes.push_back(pval.node());
        }
        // In the other case we will add the major index to dr.
        ...
        for (size_t pt_ind = 0; pt_ind<knn.size(); ++pt_ind) {
            ...
            // Get node from our cache
            {
                const auto& pc = get_local_pc(scope_nodes.at(maj_ind)->value, scope.pcname);
                for (const auto& name : scope.coords) {
                    debug("\t{} = {}", name, pc.get(name)->element<double>(min_ind));
                }
            }
            // Get node via adding to depth iterator
            {
                const auto& pc = get_local_pc(*(dr + maj_ind), scope.pcname);
                for (const auto& name : scope.coords) {
                    debug("\t{} = {}", name, pc.get(name)->element<double>(min_ind));
                }
            }
