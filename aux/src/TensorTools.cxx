...
template<typename ElementType>
PointCloud::Array make_array(const ITensor::pointer& ten)
{
    ElementType* data = (ElementType*)ten->data();
    PointCloud::Array arr(data, ten->shape(), false);
    arr.metadata() = ten->metadata();
    return arr;
}

PointCloud::Array WireCell::Aux::as_array(const ITensor::pointer& ten)
{
    const auto& dt = ten->element_type();

    if (dt == typeid(char)) return make_array<char>(ten);
    if (dt == typeid(std::byte)) return make_array<std::byte>(ten);

    if (dt == typeid(int8_t)) return make_array<int8_t>(ten);
    if (dt == typeid(int16_t)) return make_array<int16_t>(ten);
    if (dt == typeid(int32_t)) return make_array<int32_t>(ten);
    if (dt == typeid(int64_t)) return make_array<int64_t>(ten);

    if (dt == typeid(uint8_t)) return make_array<uint8_t>(ten);
    if (dt == typeid(uint16_t)) return make_array<uint16_t>(ten);
    if (dt == typeid(uint32_t)) return make_array<uint32_t>(ten);
    if (dt == typeid(uint64_t)) return make_array<uint64_t>(ten);

    if (dt == typeid(float)) return make_array<float>(ten);            
    if (dt == typeid(double)) return make_array<double>(ten);            
    if (dt == typeid(std::complex<float>)) return make_array<std::complex<float>>(ten);            
    if (dt == typeid(std::complex<double>)) return make_array<std::complex<double>>(ten);            
...
PointCloud::Dataset WireCell::Aux::as_dataset(const ITensorSet::pointer& tens)
{
    ...
    for (size_t ind=0; ind < narr; ++ind) {
        ...
        ret.add(name, std::move(as_array(sv->at(ind))));
    }
    return ret;    
}
