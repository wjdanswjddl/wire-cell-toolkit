...
implemented by HDF5.  When applied to frames, this model provides for
info-lossless persistence and potentially with no redundancy.  It
further provides flexibility to the user by mapping the frame to the
data model in one of three modes:

- sparse :: traces are saved to individual array files, potentially of
  heterogeneous sizes.

- monolithic :: all traces are saved in a single, zero-padded array
  file.

- tagged :: multiple zero-padded arrays associated with a tag are saved.

The *sparse* mode allows files to be small as possible, even without (or
especially without) file compression.  The *monolithic* mode maximally
inflates the array but compression tends to work very will with
common-value padding.  Finally, the *tagged* mode provides similar
functionality as the original /frame file/ format but avoids frame/trace
tag ambiguity and may not lead to redundant traces being saved (this
is user application dependent).  The remainder of this section
describes how a frame is decomposed into the tensor data model.
