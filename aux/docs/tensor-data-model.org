...
The *datatype* of *pctree* indicates a tensor representing an $n$-ary tree built of nodes that provide *pcnamedset*.
The tensor array of the *pctree* tensor shall be empty and the MD shall have the following attributes:

- nodes :: an object representing the $n$-ary tree.  Each attribute name is an encoding of the tree path from root node (node with no parent) to a descendant node and the value provides a *datapath* to a *pcnamedset*.

The encoding of the tree path is built from indices of children in a parent node utilizing a ~/~ separator.  For example, in a tree of three levels: L0: root node, L1: 2 children nodes, L3: 3 grandchildren per L2 node the following tree paths would exhaustively identify the nodes:

- ~0~ :: L0 root note
- ~0/{0,1}~ :: L1 children nodes
- ~0/{0,1}/{0,1,2}~ :: L2 grandchildren nodes

A *pctree* may represent an $n$-ary tree with multiple root nodes.
