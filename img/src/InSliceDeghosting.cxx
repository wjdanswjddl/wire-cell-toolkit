...
namespace {
  ...
  std::unordered_map<WireCell::WirePlaneLayer_t, std::set<int> > connected_channels(const cluster_graph_t& cg, const cluster_vertex_t& bdesc)
    {
  ...
  bool adjacent(std::unordered_map<WireCell::WirePlaneLayer_t, std::set<int> >& cid1, std::unordered_map<WireCell::WirePlaneLayer_t, std::set<int> >& cid2 ){
...
void InSliceDeghosting::local_deghosting1(const cluster_graph_t& cg, vertex_tags_t& blob_tags)
{
  ...
  for (const auto& svtx : GraphTools::mir(boost::vertices(cg))) {
	...
	if (flag_plane[it1->first]){
      ...
      blob_high_score_map[*it] = std::max(std::max(score_plane[kUlayer], score_plane[kVlayer]), score_plane[kWlayer] );
    }
	...
	if (flag_plane[it1->first]){
      ...
      blob_high_score_map[*it] = std::max(std::max(score_plane[kUlayer], score_plane[kVlayer]), score_plane[kWlayer] );
      if (exist(blob_tags,*it,GOOD))
	blob_high_score_map[*it] = 1;
    }
  ...
  //    int count_keeper = 0;
	...
	for (auto it = view_groups[2].begin(); it!=view_groups[2].end(); it++){
	  if (exist(blob_tags,*it,POTENTIAL_GOOD)){
	    ...
	    if ((!save_flag) && (cannot_remove.find(*it)==cannot_remove.end())) blob_tags.insert({*it, TO_BE_REMOVED});
	  }
	}
